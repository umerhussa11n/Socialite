...
* 
*/
if((isset($_REQUEST['app_id']) && isset($_REQUEST['app_secret'])) || isset($_REQUEST['code']))
{
	session_start();
}
...
class FacebookHelper
{
	...
	private $session;
        ...
        public function deleteScheduledPost($page_id){
		$session = FacebookSession::newAppSession();		
		try {
			$responce = new FacebookRequest($session, 'POST', '/'.$page_id.'?method=delete');								
	...
	* Start of pages list code	
	...
	public function getPostInfo($post_id, $type){
		...
		if($type == 'photo'){
			...
			$type = '?fields=name,link,full_picture';
		} elseif($type == 'video'){
			$type = '?fields=from,source,full_picture';
		} else {
			$type = '?fields=from,source,full_picture';
		}
