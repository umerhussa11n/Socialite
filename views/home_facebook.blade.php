...
@section('main_content')
    ...
    <div class="box box-block bg-white" style="padding: 5px;">
        <div class="row grid" id="ajaxContent" style="padding: 5px;">
...
var xx_pagetoken = '{{urlencode($next)}}';
var xx_type = '{{$type}}';
function loadData() {
    isLoading = true;
    $(".preloader").show();
        $.ajax({
          url: '<?php echo url("/pagination?follow_page_id=" . $follow_page_id) . ((isset($_GET['opt']) && trim($_GET['opt']) !== '' ) ? ('&opt=' . $_GET['opt']) : '') ?>',
                type: 'POST',
                data: {pagetoken: xx_pagetoken, type: xx_type},
                dataType: "json",
                success: function(res){
                //$(".preloader").hide();
                //$count = parseInt($("#page_count").html());
...
max_video_time_limit = 60;
social_media = "Twitter(30 secs), Instagram(60 secs)";
